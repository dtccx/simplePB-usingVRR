...
func (srv *PBServer) Start(command interface{}) (
	...
	// go func () {
		...
		for i, _ := range srv.peers {
			if i == srv.me {
				// replys[i].View = srv.currentView
				// replys[i].Success = true
			...
			prepareArgs := &PrepareArgs{
				View:         srv.currentView,         // the primary's current view
				PrimaryCommit:srv.commitIndex, 			   // the primary's commitIndex
				Index:        i,       	  				 // the index position at which the log entry is to be replicated on backups
